...
//attachment files path array
    ...
    public function myBooking() {
        ...
        if (!empty($_GET['q']) && ($_GET['q'] == 'a')) {
            $all_pro_booking = $this->HotelBookingDetails->find('all')->where(['user_id' => $this->Auth->user('id'), 'payment_status' => 3]);
        } elseif (!empty($_GET['q']) && ($_GET['q'] == 'p')) {
            $all_pro_booking = $this->HotelBookingDetails->find('all')->where(['user_id' => $this->Auth->user('id'), 'payment_status' => 1]);
        } elseif (!empty($_GET['q']) && ($_GET['q'] == 'c')) {
            $all_pro_booking = $this->HotelBookingDetails->find('all')->where(['user_id' => $this->Auth->user('id'), 'payment_status' => 2]);
        } else {
            $all_pro_booking = $this->HotelBookingDetails->find('all')->where(['user_id' => $this->Auth->user('id')]);
        }
        ...
        $this->set(compact('pageDetails', 'userDetails', 'pendingdet', 'pendingcount', 'past', 'pastcount', 'active', 'activecount', 'all_pro_booking'));
    }
    ...
    public function hotelajaxsearchdata() {
        ...
        if ($this->request->is('post')) {
            ...
            if (!empty($lisArr)) {
                $all_filterarray = [];
                $all_ami = $this->ExtranetsUserPropertyAmenities->find('all');
                $priceListArr = $this->ExtranetsUserPropertyPricing->find('all')->where(['price_main >=' => $data['start_price'], 'price_main <=' => $data['end_price'], 'ExtranetsUserPropertyPricing.property_id IN' => $lisArr])->extract('property_id');
            }
    ...
    public function ajaxHotelLocations() {
        if ($this->request->is('post')) {
            ...
            foreach ($data as $value) {
                ...
                echo "<li onclick='fill(&#39;" . $value['city_name'] . "&#39;,&#39;" . $datas['pos'] . "&#39;,&#39;" . $datas['hid'] . "&#39;);$(&#39;#state-name&#39;).val(&#39;" . $value['id'] . "&#39;);'><div class='city--main'>" . $value['city_name'] . " <span>" . $value['state_name'] . "</span></div><span class='sort-cart'>" . $value['country_name'] . "</span></li>";
            }
            echo "</ul><div class='hideClose' onclick='hidefill(&#39;" . $datas['hid'] . "&#39;)'><a href='javascript:void(0)'>close</a> </div> </div>";
        }
        exit;
