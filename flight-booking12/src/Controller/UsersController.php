...
//attachment files path array
    ...
    public function hotelSearchResult() {
        ...
        if ($this->request->is('get')) {
            ...
            $all_property = $this->ExtranetsUserProperty->find('all')->where(['active_ststus' => 1])->extract('id');
            $search_property = $this->ExtranetsUserPropertyLocation->find('all')->where(['city' => $datas['from_location_name']])->extract('property_id');
            $search_property_count = $this->ExtranetsUserPropertyLocation->find('all')->where(['city' => $datas['from_location_name']])->count();
           
            $intersect=[];
            if ($search_property_count>0) {
                $intersect = array_intersect($all_property->toArray(), $search_property->toArray());
            }
              ...
              $result_property=[];
              if(!empty($intersect)){
              $result_property = $this->ExtranetsUserProperty->find('all')->contain(['location','pricing','description'])->where(['ExtranetsUserProperty.id IN'=> $intersect]);
              $result_property_count = $this->ExtranetsUserProperty->find('all')->contain(['location','pricing','description'])->where(['ExtranetsUserProperty.id IN'=> $intersect])->count();
              
              }
              $this->set(compact('result_property','result_property_count'));
...
//                echo "already exit";
    ...
    public function ajaxLocations() {
        if ($this->request->is('post')) {
            ...
            echo "</ul><div class='hideClose' onclick='hidefill(&#39;" . $datas['hid'] . "&#39;)'><a href='javascript:void(0)'>close</a> </div> </div>";
        }
        // $locations = [];
    ...
    public function ajaxHotelLocations() {
        if ($this->request->is('post')) {
            ...
            foreach ($data as $value) {
                ...
                echo "<li onclick='fill(&#39;" . $value['city_name'] . "&#39;,&#39;" . $datas['pos'] . "&#39;,&#39;" . $datas['hid'] . "&#39;)'><div class='city--main'>" . $value['city_name'] . " <span>" . "</span></li>";
            }
            echo "</ul><div class='hideClose' onclick='hidefill(&#39;" . $datas['hid'] . "&#39;)'><a href='javascript:void(0)'>close</a> </div> </div>";
        }
        exit;
